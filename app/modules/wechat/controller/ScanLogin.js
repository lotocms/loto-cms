...
class ScanLoginController extends Chan.Controller {
  ...
  async loginQrCode(req, res) {
    try {
      ...
      // 存储扫码状态
      await db("wechat_scan_login").insert({
        scan_id: scanId,
        status: 0, // 0:未扫码
      ...
      res.status(500).json(this.fail({ msg: "二维码生成失败，请稍后重试" }));
    }
  }
  ...
  async scanStatus(req, res) {
    try {
      ...
      if (!scanId) {
        return res.status(400).json(Chan.common.fail({ msg: "缺少scanId参数" }));
      }
      ...
      switch (scanRecord.status) {
        case 0:
          return res.json(this.success({ data: { status: 0, code: "SCAN_PENDING" } }));
        case 1: {
        ...
        case 2:
          return res.json(this.success({ data: { status: 2, code: "SCAN_ALREADY_LOGIN" } }));
        default:
          return res.json(Chan.common.fail({ msg: "未知状态", data: { code: "UNKNOWN_STATUS" } }));
      }
    } catch (error) {
      console.error("轮询扫码状态失败:", error);
      res.status(500).json(Chan.common.fail({ msg: "查询登录状态失败" }));
    }
  }
}
