...
import { cleanHtml, getValueByPath, arrayToHtml } from "../../../middleware/clear-html.js";
class GatherController extends Chan.Controller {
  ...
  async getArticle(req, res, next) {
    try {
      ...
      if (!targetUrl) {
        return res.json(this.fail({ msg: "缺少 targetUrl" }));
      }
      if (!parseData) {
        return res.json(this.fail({ msg: "缺少 parseData 配置" }));
      }
      ...
      try {
        ...
        return res.json(this.fail({ msg: "parseData 不是合法 JSON" }));
      }
  ...
  async detail(req, res, next) {
    try {
      ...
      res.json(this.success({ data: data.data }));
    } catch (err) {
      next(err);
    }
  }
